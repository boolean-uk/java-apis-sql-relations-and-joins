...
SELECT films.title,directors.director_name,stars.star_name,writers.writer_name FROM films JOIN writers ON films.writer_id = writers.id JOIN stars ON films.star_id = stars.id JOIN directors ON films.director_id = directors.id WHERE year>=1980; KEY (person) REFERENCES people(id);